...
export function createLibraryView({ store, onChoose, selectedId, t, locale }) {
  ...
  function expanded(value) { toggle.setAttribute('aria-expanded',String(value)); toggle.setAttribute('aria-label',value?t('library.close'):t('library.open')); }
  function close() {
  ...
  function refresh() {
    ...
    for(const entry of results){
      ...
      row.append(node('p',entry.lessonCount+' '+t('lessons')+' · '+formatDuration(entry.selectedDurationSeconds),'library-meta'));
      row.append(node('p',t('priorityDuration',{duration:formatDuration(entry.priorityDurationSeconds)}),'library-meta'));
      const progress=node('progress');progress.max=entry.lessonCount;progress.value=entry.completed;progress.setAttribute('aria-label',t('progressOf',{title:entry.title}));
      row.append(progress,node('p',t('progress',{count:entry.completed,total:entry.lessonCount})+(entry.liked?t('likeSuffix'):''),'library-meta'));
      if(entry.matchingLessons.length){const matches=node('ul',undefined,'library-matches');for(const lesson of entry.matchingLessons){const item=node('li');item.append(link(t('match',{order:lesson.order,title:lesson.title}),entry,lesson));matches.append(item);}row.append(matches);}
      list.append(row);
    }
    $('libraryResults').replaceChildren(list);
    $('libraryCount').textContent=t('libraryCount',{count:results.length,label:t(results.length===1?'materialOne':'materialMany')});
  ...
  return {refresh,setData(library){
    ...
    $('languageFilter').replaceChildren(new Option(t('library.allLanguages'),''),...languages.map(language=>new Option(new Intl.DisplayNames([locale()],{type:'language'}).of(language),language)));
    const used=new Set(index.materials.filter(m=>items.some(i=>i.material.id===m.id)).flatMap(m=>m.topics));
    $('topicFilter').replaceChildren(new Option(t('library.allTopics'),''),...index.topics.filter(t=>used.has(t.id)).map(t=>new Option(t.label,t.id)));
    refresh();
  },setLocale(){
    if(items.length) this.setData({materials:items,index});
    else refresh();
  }};
}
